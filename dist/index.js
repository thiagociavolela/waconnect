...
const whatsapp = new whatsapp_1.WhatsAppService();
...
app.post("/api/send/text", async (req, res) => {
    ...
    if (idemKey && replyIfCached(idemKey, res))
        return;
    if (!to || !message) {
...
app.post("/api/send/media", upload.single("file"), async (req, res) => {
    ...
    const kind = kindInput ?? deduceKind(mimetype);
    try {
...
app.post("/api/send/narration", async (req, res) => {
    ...
    try {
        ...
        cacheIdem(idemKey, res, { error: "Falha ao enviar áudio narrado.", detail: err?.message }, 500);
    }
});
...
function getIdemKey(req) {
    return req.headers["idempotency-key"] ?? req.body?.clientMessageId ?? null;
}
function replyIfCached(key, res) {
    if (!key)
        return false;
...
function cacheIdem(key, res, body, status = 200) {
    if (key) {
